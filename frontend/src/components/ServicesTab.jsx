...
function ServiceTab({ phoneNumber, setError, setSuccess }) {
  const [serviceTab, setServiceTab] = useState(0); // 0: Tạo chú thích, 1: Tạo ý tưởng
  ...
  const handleGenerateCaptions = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.captions) {
        setCaptions(data.captions.map((caption) => ({ caption })));
        setSuccess("Đã tạo 5 chú thích thành công!");
      } else {
        setError(data.error || "Không thể tạo chú thích. Vui lòng thử lại.");
      }
    } catch (error) {
  ...
  const handleGeneratePostIdeas = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/GetPostIdeas`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ topic, phoneNumber }),
      });
  ...
  return (
    <Box>
      ...
      {/* Tab con trong Services */}
      <Box sx={{ mb: 4, borderBottom: 1, borderColor: "divider" }}>
        <Tabs
          value={serviceTab}
          onChange={(e, newValue) => setServiceTab(newValue)}
          centered
        >
          <Tab label="Tạo chú thích từ đầu" />
          <Tab label="Tạo ý tưởng bài đăng" />
        </Tabs>
      </Box>
      ...
      {serviceTab === 0 && (
        <Box>
          ...
          <Box component="form" onSubmit={handleGenerateCaptions} sx={{ mb: 4 }}>
            ...
            <TextField
              ...
              fullWidth
              margin="normal"
              placeholder="VD: Du lịch, Ẩm thực, Công nghệ"
            />
            <FormControl fullWidth margin="normal">
      ...
      {serviceTab === 1 && (
        <Box>
          ...
          <Box component="form" onSubmit={handleGeneratePostIdeas} sx={{ mb: 4 }}>
            <TextField
              label="Chủ đề"
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
                  ...
                  <ListItem key={index} sx={{ mb: 2 }}>
                    <Card sx={{ width: "100%" }}>
                      ...
                      <CardActions>
                        ...
                        <IconButton
                          onClick={() => handleShareCaption({ idea, caption })}
                          color="primary"
                          title="Chia sẻ chú thích"
                        >
                          <Share />
